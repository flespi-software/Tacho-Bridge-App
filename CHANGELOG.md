...
[feature] Pre-release builds (alpha/beta/rc) now report their exact git commit as semver build metadata (e.g. 0.8.0-alpha.6+1a2b3c4) in the startup log and in the settings report on the server — a republished version is now traceable to the exact build. Stable versions stay clean.  