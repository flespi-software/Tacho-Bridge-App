<template>
    <div>
        <q-list
            ...
            style="width: 600px; padding-bottom: 10px"
        >
            ...
            <q-toolbar
                inset
                v-for="(reader, index) in state.readers"
                :key="index"
            >
                ...
                <q-item-section top side>
                    <div class="text-grey-8 q-gutter-xs">
                        ...
                        <q-dialog
                            v-model="EnterCardNumberDialog"
                            class="card-number-dialog"
                        >
                            <q-card>
                                <q-card-section>
                                    <q-input
    ...
    if (index !== -1) {
        ...
        state.readers[index] = {
            ...
            online:
                payload.online !== null
                    ? payload.online
                    : state.readers[index].online,
            authentication:
                payload.authentication !== null
                    ? payload.authentication
                    : state.readers[index].authentication,
        };
    } else {
...
const saveCardNumber = async (cardATR: string) => {
    ...
    const readerIndex = state.readers.findIndex(
        (reader) => reader.cardATR === cardATR
    );
    ...
    let update_result = await invoke('update_card', {
        atr: cardATR,
